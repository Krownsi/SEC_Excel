...
function main()
	Dim gettickerUrl, tickerHttpRequest, tickerHttpResponse, Token, timeInterval, tickerParser, numberOfTickers, currentTicker, stockName, stockCurrency, tickerSearch, stockLetter, letterZ

	Token = "WSTRKXB9H1YB1KWZ"
	letterZ = 90
	
	'Delete all preexisting files.
		Set objDeleteAll = CreateObject("Scripting.FileSystemObject")
		objDeleteAll.DeleteFile("C:\Users\Alex\Documents\StockExcelSpreadSheets\StockSpreadSheets\*"), true
	
	For stockLetter = 65 To letterZ
		'Get all possible tickers to check.
		gettickerUrl = "https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=" & Chr(stockLetter) & "&apikey="&Token&"&datatype=json"
		...
		'Create JSON parser.
		Set tickerParser = New JSONobject
		tickerParser.parse(tickerHttpResponse)
