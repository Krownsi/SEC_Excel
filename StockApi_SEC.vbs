...
class JSONobject
	...
	public function parse(byval strJson)
		...
		dim tmpArray, tmpObj, addedToArray
...
function main()
	Dim gettickerUrl, tickerHttpRequest, tickerHttpResponse, timeInterval, tickerParser, numberOfTickers, currentTicker, stockName, stockCurrency, tickerSearch, spacelessTicker

	'Make request to the SEC - returns every single ticker and corresponding CLK value.
	gettickerUrl = "https://www.sec.gov/include/ticker.txt"
	Set tickerHttpRequest = CreateObject("MSXML2.XMLHTTP.6.0")
	tickerHttpRequest.open "GET", gettickerUrl, False
	tickerHttpRequest.send
	
	'Replace all non numbers.
	Set regExpLet = New RegExp
	regExpLet.IgnoreCase = True
	regExpLet.Global = True
	
	'Add all non characters that you don't want to switch.
	regExpLet.Pattern = "[^a-zA-Z]" 
	
	tickerHttpResponse = tickerHttpRequest.responseText
	spacelessTicker = Split(CStr(tickerHttpResponse), "	")
	
	Dim CLKValue, ticker, token
	
	token = "WSTRKXB9H1YB1KWZ"
	
	'Go through each ticker.
	For Each ticker In spacelessTicker
		TickerValue = regExpLet.Replace(ticker, "")
				
		If StrComp(CLKValue, "") <> 1 Then
			
			'CLKValue is the CLK number for the specific ticker.
			
			'////////////////////////////////////////////////////////////////////////////////////////////////////////
			'////	Get Indicator Information (from alphavantage API) for ticker.
			'////////////////////////////////////////////////////////////////////////////////////////////////////////
			'//// Starting out we will have two moving average (SMA) indicators. With these, we will simply check the 
			'//// when a possible cross is likely.
			'////////////////////////////////////////////////////////////////////////////////////////////////////////
			
			gettickerUrl = "https://www.alphavantage.co/query?function=SMA&symbol=" & CStr(TickerValue) & "&interval=5min&apikey="&Token&"&datatype=json&time_period=40&series_type=close"
			Set tickerHttpRequest = CreateObject("MSXML2.XMLHTTP.6.0")
			tickerHttpRequest.open "GET", gettickerUrl, False
			tickerHttpRequest.send
	...
	Set regExpLet = Nothing
	
	
	
	
	
	numberOfTickers = tickerParser("bestMatches").Length
	
	'Default.
	timeInterval = "1min"
	
	For currentTicker = 0 To numberOfTickers - 1
	'For each ticker, we must get information and send it.
	
		stockTicker = tickerParser("bestMatches")(currentTicker)("1. symbol")
		stockName = tickerParser("bestMatches")(currentTicker)("2. name")
		stockCurrency = tickerParser("bestMatches")(currentTicker)("8. currency")
		
		'Create Excel file.
		ExcelStockMaker stockTicker, stockName, stockCurrency, Token, timeInterval
...
Function ExcelStockMaker(stockTicker, stockName, stockCurrency, Token, timeInterval)
	Dim done
	
	'Set up string.
	Dim UrlRequest, StockRequest, StockResponse, oXMLHTTP, TokenKey, tempStockHolder
		
	UrlRequest = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol="&StockTicker&"&apikey="&token&"&interval=" & timeInterval & "&outputsize=full&datatype=json"
	Set oXMLHTTP = CreateObject("MSXML2.XMLHTTP.6.0")
	oXMLHTTP.open "GET", UrlRequest, False
	oXMLHTTP.send
		
	If oXMLHTTP.status = 200 Then
		StockResponse = oXMLHTTP.responseText
	Else
		Exit Function
	End if
	
	'Create excel file.
	Dim strFileName, objExcelCreate, objWorkBookCreate, checkIfFileExists, fileExists
		
	'Open excel file and write into it.
	strFileName = "C:\Users\Alex\Documents\StockExcelSpreadSheets\StockSpreadSheets\" & stockTicker & "_" & stockCurrency & ".xlsx"
	
	'Check if file already exists - if not, leave and add.
	Set checkIfFileExists = CreateObject("Scripting.FileSystemObject")
	
	fileExists = checkIfFileExists.FileExists(strFileName)
	
	Set objExcelCreate = CreateObject("Excel.Application")
	objExcelCreate.Visible = True
	Set objWorkBookCreate = objExcelCreate.Workbooks.Add()
	objWorkBookCreate.SaveAs(strFileName)
	objExcelCreate.Quit
	
	'Reset
	Set tempStockHolder = Nothing
	
	'Set up input into SQL server.
	Set tempStockHolder = New JSONobject
	tempStockHolder.parse(StockResponse)
	
	Dim objExcelAdd, objWorkBookAdd
	
	Set objExcelAdd = CreateObject("Excel.Application")	
	Set objWorkBookAdd = objExcelAdd.Workbooks.Open(strFileName)
	objExcelAdd.Application.Visible = True
	objWorkBookAdd.Worksheets(1).Activate	
	
	Dim stockHigh, stockLow, stockClose, stockOpen, stockVolume, stockJsonInformation, eachStockTime, stockTiming, incrementer

	'stockJsonInformation = tempStockHolder("Time Series (" & timeInterval & ")").pairs
	
	incrementer = 0
	'For Each eachStockTime In stockJsonInformation
	'	incrementer = incrementer + 1
	'	
	'	'Get the exact timing.
	'	stockTiming = eachStockTime.name
	'	
	'	'Separate the information.
	'	stockOpen = tempStockHolder("Time Series (" & timeInterval & ")")(stockTiming)("1. open")
	'	stockHigh = tempStockHolder("Time Series (" & timeInterval & ")")(stockTiming)("2. high")
	'	stockLow = tempStockHolder("Time Series (" & timeInterval & ")")(stockTiming)("3. low")
	'	stockClose = tempStockHolder("Time Series (" & timeInterval & ")")(stockTiming)("4. close")
	'	stockVolume = tempStockHolder("Time Series (" & timeInterval & ")")(stockTiming)("5. volume")
	'
	'	'Add to Excel spread sheet.
	'	If incrementer = 1 then
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 1).Value = stockTicker
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 2).Value = "Open"
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 3).Value = "Close"
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 4).Value = "High"
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 5).Value = "Low"
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 6).Value = "Volume"
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 8).Value = "END OF DAY"
	'		objWorkBookAdd.Worksheets(1).Cells(incrementer, 10).Value = CurrentDate
	'	End If
	'	
	'	objWorkBookAdd.Worksheets(1).Cells(incrementer + 2, 2).Value = stockOpen
	'	objWorkBookAdd.Worksheets(1).Cells(incrementer + 2, 3).Value = stockClose
	'	objWorkBookAdd.Worksheets(1).Cells(incrementer + 2, 4).Value = stockHigh
	'	objWorkBookAdd.Worksheets(1).Cells(incrementer + 2, 5).Value = stockLow
	'	objWorkBookAdd.Worksheets(1).Cells(incrementer + 2, 6).Value = stockVolume
	'	
	'	objWorkBookAdd.Worksheets(1).Cells(incrementer + 2, 8).Value = stockTiming
	'	
'
'	Next 
	
	objWorkBookAdd.Worksheets(1).Cells(incrementer + 4, 1).Value = stockTicker
	objWorkBookAdd.Worksheets(1).Cells(incrementer + 4, 2).Value = "Open"
	objWorkBookAdd.Worksheets(1).Cells(incrementer + 4, 3).Value = "Close"
	objWorkBookAdd.Worksheets(1).Cells(incrementer + 4, 4).Value = "High"
	objWorkBookAdd.Worksheets(1).Cells(incrementer + 4, 5).Value = "Low"
	objWorkBookAdd.Worksheets(1).Cells(incrementer + 4, 6).Value = "Volume"
	objWorkBookAdd.Worksheets(1).Cells(incrementer + 4, 8).Value = "START OF DAY"
	
	objExcelAdd.ActiveWorkbook.Save
	objExcelAdd.ActiveWorkbook.Close
	
	objExcelAdd.Application.Quit
